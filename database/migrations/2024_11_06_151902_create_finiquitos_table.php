...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('finiquitos', function (Blueprint $table) {
            ...
            $table->foreign('trabajador_id')->references('id_itc')->on('trabajadores')->onDelete('set null')->onUpdate('cascade');

            $table->string('contrato_id');
            $table->foreign('contrato_id')->references('id_contrato')->on('contratos')->onDelete('restrict')->onUpdate('cascade');

            $table->string('compania_id')->nullable();
            $table->foreign('compania_id')->references('id_compania')->on('companias')->onDelete('set null')->onUpdate('cascade'); //Llave foranea de Companias
